...
/// <summary>
/// 방향마다 다른 그림 묶음. 가리킨 쪽이 비어 있으면 직각 쪽 → default → 대체 목록 순으로 떨어진다.
...
public sealed class DirectionalSprites
{
    ...
    /// <summary>
    /// 이 쪽에 쓸 후보. 가리킨 쪽이 없으면 직각 쪽으로 간다.
    /// 좌우만 그려둔 캐릭터가 위로 걸을 때 정면으로 굳지 않게, 그때 같이 밀린 좌우 그림을 쓴다.
    /// 그마저 없으면 default, 그것도 없으면 대체 목록.
    /// </summary>
    public IReadOnlyList<Sprite> For(Aim aim)
    {
        if (_byDirection.TryGetValue(aim.Primary, out IReadOnlyList<Sprite>? matched))
        {
            return matched;
        }

        if (_byDirection.TryGetValue(aim.Cross, out IReadOnlyList<Sprite>? near))
        {
            return near;
        }
